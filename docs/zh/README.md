...
RorisDB 是一个**通用数据库仿真平台**，可以同时使用**14种不同的数据库协议**。使用 Rust 和 Apache DataFusion 构建，它是世界上第一个可以替代以下数据库的系统：
...
就像变色龙适应环境一样，RorisDB 适应**任何数据库协议**：
...
[网站](https://rorisdb.io) · [博客](https://blog.rorisdb.io) · [Twitter](https://twitter.com/rorisdb) · [Discord](https://discord.gg/rorisdb)
