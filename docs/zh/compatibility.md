# HarnessDB — Doris 兼容性说明

> 版本 0.3.0

HarnessDB 目标是**SQL 级别的兼容性**，同时使用完全不同的内部架构（DataFusion + Parquet 而非自研存储引擎）。

## SQL 兼容性概要

| 类别 | Doris | HarnessDB | 兼容性 |
|------|-------|---------|--------|
...
HarnessDB 将所有查询执行委托给 **Apache DataFusion**，提供优秀的 SQL 覆盖：

| 功能 | Doris | HarnessDB | 说明 |
|------|-------|---------|------|
