...
fn test_string_concat() {
    ...
    let result = fe_sql_parser::parse_sql(
        "SELECT CONCAT(name, ' works in ', department) AS info FROM employees",
    );
    assert!(result.is_ok());
}
...
fn test_string_substring() {
    let catalog = common::create_test_catalog();
    let result =
        fe_sql_parser::parse_sql("SELECT SUBSTRING(name, 1, 3) AS short_name FROM employees");
    assert!(result.is_ok());
}
...
fn test_string_trim_ltrim_rtrim() {
    let catalog = common::create_test_catalog();
    let result =
        fe_sql_parser::parse_sql("SELECT TRIM(name), LTRIM(name), RTRIM(name) FROM employees");
    assert!(result.is_ok());
}
...
fn test_string_replace() {
    let catalog = common::create_test_catalog();
    let result =
        fe_sql_parser::parse_sql("SELECT REPLACE(department, 'Engineering', 'Eng') FROM employees");
    assert!(result.is_ok());
}
...
fn test_string_lpad_rpad() {
    let catalog = common::create_test_catalog();
    let result =
        fe_sql_parser::parse_sql("SELECT LPAD(name, 10, '*'), RPAD(name, 10, '-') FROM employees");
    assert!(result.is_ok());
}
...
fn test_string_locate_instr() {
    let catalog = common::create_test_catalog();
    let result =
        fe_sql_parser::parse_sql("SELECT LOCATE('li', name), INSTR(name, 'a') FROM employees");
    assert!(result.is_ok());
}
...
fn test_math_abs_ceil_floor_round() {
    let catalog = common::create_test_catalog();
    let result = fe_sql_parser::parse_sql(
        "SELECT ABS(-5), CEIL(3.2), FLOOR(3.8), ROUND(3.14159, 2) FROM employees",
    );
    assert!(result.is_ok());
}
...
fn test_math_trigonometric() {
    let catalog = common::create_test_catalog();
    let result = fe_sql_parser::parse_sql(
        "SELECT SIN(0), COS(0), TAN(0), ASIN(0), ACOS(1), ATAN(0) FROM employees",
    );
    assert!(result.is_ok());
}
...
fn test_math_log_exp_sqrt_pow() {
    let catalog = common::create_test_catalog();
    let result = fe_sql_parser::parse_sql(
        "SELECT LOG(2.718), LOG10(100), EXP(1), SQRT(16), POW(2, 10) FROM employees",
    );
    assert!(result.is_ok());
}
...
fn test_math_greatest_least() {
    let catalog = common::create_test_catalog();
    let result = fe_sql_parser::parse_sql(
        "SELECT GREATEST(salary, 100000), LEAST(salary, 80000) FROM employees",
    );
    assert!(result.is_ok());
}
...
fn test_date_extract_components() {
    let catalog = common::create_test_catalog();
    let result = fe_sql_parser::parse_sql(
        "SELECT YEAR('2024-01-15'), MONTH('2024-06-15'), DAY('2024-01-15') FROM employees",
    );
    assert!(result.is_ok());
}
...
fn test_date_hour_minute_second() {
    let catalog = common::create_test_catalog();
    let result = fe_sql_parser::parse_sql(
        "SELECT HOUR('2024-01-15 14:30:45'), MINUTE('2024-01-15 14:30:45'), SECOND('2024-01-15 14:30:45') FROM employees",
    );
    assert!(result.is_ok());
}
...
fn test_date_datediff() {
    let catalog = common::create_test_catalog();
    let result =
        fe_sql_parser::parse_sql("SELECT DATEDIFF('2024-12-31', '2024-01-01') FROM employees");
    assert!(result.is_ok());
}
...
fn test_date_add_sub() {
    let catalog = common::create_test_catalog();
    let result = fe_sql_parser::parse_sql(
        "SELECT DATE_ADD('2024-01-15', INTERVAL 30 DAY), DATE_SUB('2024-01-15', INTERVAL 1 MONTH) FROM employees",
    );
    assert!(result.is_ok());
}
...
fn test_date_format() {
    let catalog = common::create_test_catalog();
    let result =
        fe_sql_parser::parse_sql("SELECT DATE_FORMAT('2024-01-15', '%Y-%m') FROM employees");
    assert!(result.is_ok());
}
...
fn test_date_now_curdate() {
    let catalog = common::create_test_catalog();
    let result = fe_sql_parser::parse_sql(
        "SELECT NOW(), CURDATE(), CURRENT_TIMESTAMP, CURRENT_DATE FROM employees",
    );
    assert!(result.is_ok());
}
...
fn test_date_week_quarter() {
    let catalog = common::create_test_catalog();
    let result = fe_sql_parser::parse_sql(
        "SELECT WEEK('2024-01-15'), QUARTER('2024-06-15'), MONTHNAME('2024-01-15'), DAYNAME('2024-01-15') FROM employees",
    );
    assert!(result.is_ok());
}
...
fn test_json_extract() {
    let catalog = common::create_test_catalog();
    let result = fe_sql_parser::parse_sql(
        "SELECT JSON_EXTRACT('{\"a\": 1, \"b\": 2}', '$.a') FROM employees",
    );
    assert!(result.is_ok());
}
...
fn test_json_contains() {
    let catalog = common::create_test_catalog();
    let result = fe_sql_parser::parse_sql(
        "SELECT JSON_CONTAINS('{\"a\": 1, \"b\": 2}', '1', '$.a') FROM employees",
    );
    assert!(result.is_ok());
}
...
fn test_json_array_object() {
    let catalog = common::create_test_catalog();
    let result = fe_sql_parser::parse_sql(
        "SELECT JSON_ARRAY(1, 2, 3), JSON_OBJECT('key', 'value') FROM employees",
    );
    assert!(result.is_ok());
}
...
fn test_json_length_keys() {
    let catalog = common::create_test_catalog();
    let result = fe_sql_parser::parse_sql(
        "SELECT JSON_LENGTH('{\"a\": 1, \"b\": 2}'), JSON_KEYS('{\"a\": 1, \"b\": 2}') FROM employees",
    );
    assert!(result.is_ok());
}
...
fn test_json_valid() {
    let catalog = common::create_test_catalog();
    let result = fe_sql_parser::parse_sql(
        "SELECT JSON_VALID('{\"a\": 1}'), JSON_VALID('not json') FROM employees",
    );
    assert!(result.is_ok());
}
...
fn test_coalesce() {
    let catalog = common::create_test_catalog();
    let result =
        fe_sql_parser::parse_sql("SELECT COALESCE(department, 'Unassigned') FROM employees");
    assert!(result.is_ok());
}
...
fn test_ifnull_nullif() {
    let catalog = common::create_test_catalog();
    let result = fe_sql_parser::parse_sql(
        "SELECT IFNULL(department, 'N/A'), NULLIF(department, 'Sales') FROM employees",
    );
    assert!(result.is_ok());
}
...
fn test_if_function() {
    let catalog = common::create_test_catalog();
    let result =
        fe_sql_parser::parse_sql("SELECT IF(salary > 80000, 'high', 'low') FROM employees");
    assert!(result.is_ok());
}
...
fn test_bitwise_shift() {
    ...
    let result =
        fe_sql_parser::parse_sql("SELECT BITSHIFTL(id, 2), BITSHIFTR(id, 1) FROM employees");
    assert!(result.is_ok());
}
