...
use fe_catalog::table::{KeysType, Table, TableColumn};
...
impl HarnessQueryHandler {
    ...
    pub(crate) fn create_table(
        ...
        use fe_catalog::table::{
            DistributionInfo, KeysType, PartitionInfo, Table, TableColumn,
        };

        let columns: Vec<TableColumn> = stmt
            .columns
            .iter()
            .map(|c| TableColumn {
                name: c.name.clone(),
        ...
        let distribution_info = stmt.distribution.as_ref().map(|d| DistributionInfo {
            dist_type: d.dist_type.clone(),
            columns: d.columns.clone(),
            buckets: d.buckets as u32,
        });

        let properties: HashMap<String, String> = stmt.properties.iter().cloned().collect();

        let replication_num = properties
            .get("replication_num")
        ...
        match catalog.create_table(db, table) {
            Ok(()) => {
                ...
                let _ = catalog;
                let arrow_fields: Vec<datafusion::arrow::datatypes::Field> = stmt
                    .columns
                    .iter()
                    .map(|c| {
                        datafusion::arrow::datatypes::Field::new(
                            &c.name,
                            fe_datafusion::types::to_arrow_data_type(&parse_data_type(
                                &c.data_type,
                            )),
                            c.nullable,
                        )
                    ...
                    match op {
                        ...
                        fe_sql_parser::ast::AlterOperation::AddPartition {
                            ...
                            table.properties.insert(
                                format!("__partition_{}_values", partition_name),
                                values_less_than.join(","),
                            );
                            for (k, v) in properties {
                                table.properties.insert(
                                    format!("__partition_{}_{}", partition_name, k),
                                    v.clone(),
                                );
                            }
                            catalog.create_table(db, table).map_err(|e| e.to_string())?;
                        ...
                        fe_sql_parser::ast::AlterOperation::DropPartition {
                            ...
                            let parts: Vec<&str> =
                                list.split(',').filter(|p| *p != partition_name).collect();
                        ...
                        fe_sql_parser::ast::AlterOperation::AddRollup {
                            ...
                            table.properties.insert(key, list);
                            table.properties.insert(
                                format!("__rollup_{}_columns", rollup_name),
                                columns.join(","),
                            );
                            for (k, v) in properties {
                                table
                        ...
                        fe_sql_parser::ast::AlterOperation::DropRollup {
                            ...
                            let list = table.properties.get(&key).cloned().unwrap_or_default();
                            let parts: Vec<&str> =
                                list.split(',').filter(|p| *p != rollup_name).collect();
                            if parts.len() == list.split(',').count()
                                && !list.is_empty()
                                && !if_exists
                            {
                            ...
                            if let Some(mut table) = catalog.get_table(db, &stmt.table) {
                                ...
                                if *swap {
                                    if let Some(mut old_tbl) = catalog.get_table(db, old_table) {
                                        ...
                                        catalog
                                            .create_table(db, old_tbl)
                                            .map_err(|e| e.to_string())?;
                                    }
                                } else {
                                    catalog
                                        .drop_table(db, old_table)
                                        .map_err(|e| e.to_string())?;
                                }
                            }
                        }
                        ...
                        fe_sql_parser::ast::AlterOperation::AddColumn(col_def) => {
                            ...
                            let new_col = TableColumn {
                                ...
                                data_type: parse_data_type(&col_def.data_type),
                                nullable: col_def.nullable,
                                default_value: col_def.default_value.as_ref().and_then(
                                    |e| match e {
                                        fe_sql_parser::ast::Expr::Literal(lit) => {
                                            Some(literal_to_string(lit))
                                        }
                                        _ => None,
                                    },
                                ),
                            ...
                            if let Err(e) =
                                self.storage
                                    .rewrite_parquet_add_column(db, &stmt.table, &new_field)
                            {
                        ...
                        fe_sql_parser::ast::AlterOperation::DropColumn(col_name) => {
                            ...
                            if let Some(idx) = idx {
                                ...
                                if let Err(e) =
                                    self.storage
                                        .rewrite_parquet_drop_column(db, &stmt.table, idx)
                                {
                                    tracing::warn!(
                                        "Failed to rewrite parquet dropping column: {}",
                                        e
                                    );
                                }
    ...
    pub(crate) fn truncate_table(
        ...
        match catalog.get_table(db, &table) {
            Some(tbl) => {
                ...
                let arrow_fields: Vec<datafusion::arrow::datatypes::Field> = tbl
                    .columns
                    .iter()
                    .map(|c| {
                        datafusion::arrow::datatypes::Field::new(
    ...
    pub(crate) fn drop_view(
        ...
        let idx = views
            .iter()
            .position(|v| v.database == db && v.name == stmt.name);
        match idx {
            Some(i) => {
                views.remove(i);
                Ok(QueryResult::ok())
    ...
    pub(crate) fn alter_view(
        ...
        let view = views
            .iter_mut()
            .find(|v| v.database == db && v.name == stmt.name)
    ...
    pub(crate) fn create_index(
        ...
        match catalog.get_table(db, &stmt.table) {
            Some(_) => {
                ...
                if let Some(mut table) = catalog.get_table(db, &stmt.table) {
                    table.properties.insert(
                        format!("__index_{}", stmt.index_name),
                        stmt.columns.join(","),
                    );
                    if let Some(ref itype) = stmt.index_type {
                        table
                            .properties
                            .insert(format!("__index_{}_type", stmt.index_name), itype.clone());
                    }
                    catalog.create_table(db, table).map_err(|e| e.to_string())?;
                }
                Ok(QueryResult::ok())
                ...
                if let Some(mut table) = catalog.get_table(db, &stmt.table) {
                    ...
                    if !table.properties.contains_key(&key) && !stmt.if_exists {
                        return Err(format!(
                            ...
                            stmt.index_name, db, stmt.table
                        ));
                    }
                    table.properties.remove(&key);
                    table
                        .properties
                        .remove(&format!("__index_{}_type", stmt.index_name));
                    catalog.create_table(db, table).map_err(|e| e.to_string())?;
                }
                Ok(QueryResult::ok())
            }
    ...
    pub(crate) fn alter_materialized_view(
        ...
        Ok(QueryResult::with_rows(
            ...
            vec![vec![Some(format!(
                "ALTER MATERIALIZED VIEW {}.{} OK",
                stmt.database.as_deref().unwrap_or(&String::new()),
                stmt.name
            ))]],
        ))
    }
    ...
    pub(crate) fn create_repository(
        ...
        Ok(QueryResult::with_rows(
            ...
            vec![vec![Some(format!(
                "CREATE REPOSITORY `{}` at '{}' completed",
                stmt.name, path
            ))]],
        ))
    }

    pub(crate) fn drop_repository(
        &self,
        _conn_id: u32,
    ...
    pub(crate) fn cancel_alter_table(
        &self,
        conn_id: u32,
        stmt: &CancelAlterTableStmt,
    ) -> Result<QueryResult, String> {
        let catalog = &self.catalog;
        let current_db = self.get_session(conn_id);
        let db = stmt.database.as_deref().unwrap_or(&current_db);
    ...
    pub(crate) fn alter_colocate_group(
        &self,
        conn_id: u32,
        stmt: &AlterColocateGroupStmt,
    ) -> Result<QueryResult, String> {
        ...
        match &stmt.operation {
            ...
            ColocateGroupOperation::SetProperty(props) => Ok(QueryResult::with_rows(
                vec![ColumnDef {
                    name: "Status".to_string(),
                    col_type: ColumnType::String,
                }],
                vec![vec![Some(format!(
                    "Colocate group '{}' properties updated ({} properties)",
                    stmt.group_name,
                    props.len()
                ))]],
            )),
    ...
    pub(crate) fn create_user(
        &self,
        _conn_id: u32,
        stmt: &fe_sql_parser::ast::CreateUserStmt,
    ) -> Result<QueryResult, String> {
    ...
    pub(crate) fn drop_user(
        &self,
        _conn_id: u32,
        stmt: &fe_sql_parser::ast::DropUserStmt,
    ) -> Result<QueryResult, String> {
        ...
        Ok(QueryResult::with_rows(
            ...
            vec![vec![Some(if stmt.if_exists {
                format!("DROP USER {} OK (if exists)", stmt.username)
            } else {
                format!("DROP USER {} OK", stmt.username)
            })]],
        ))
    }
    ...
    pub(crate) fn create_catalog(
        &self,
        _conn_id: u32,
        stmt: &fe_sql_parser::ast::CreateCatalogStmt,
    ) -> Result<QueryResult, String> {
        Ok(QueryResult::with_rows(
            vec![ColumnDef {
                name: "Status".to_string(),
    ...
    pub(crate) fn drop_catalog(
        &self,
        _conn_id: u32,
        stmt: &fe_sql_parser::ast::DropCatalogStmt,
    ) -> Result<QueryResult, String> {
        Ok(QueryResult::with_rows(
            vec![ColumnDef {
                name: "Status".to_string(),
    ...
    pub(crate) fn refresh_catalog(
        &self,
        _conn_id: u32,
        stmt: &fe_sql_parser::ast::RefreshCatalogStmt,
    ) -> Result<QueryResult, String> {
        Ok(QueryResult::with_rows(
            vec![ColumnDef {
                name: "Status".to_string(),
    ...
    pub(crate) fn export_table(
        &self,
        conn_id: u32,
        stmt: &fe_sql_parser::ast::ExportTableStmt,
    ) -> Result<QueryResult, String> {
        let current_db = self.get_session(conn_id);
        let db = stmt.database.as_deref().unwrap_or(&current_db);
        let format = stmt
        ...
        let msg =
            fe_backup::export::export_table(&self.storage, db, &stmt.table, &stmt.path, format)?;
        Ok(QueryResult::with_rows(
            vec![ColumnDef {
                name: "Status".to_string(),
                col_type: ColumnType::String,
    ...
    pub(crate) fn set_variable(
        &self,
        conn_id: u32,
        stmt: &fe_sql_parser::ast::SetVariableStmt,
    ) -> Result<QueryResult, String> {
        ...
        let value = match &stmt.value {
            fe_sql_parser::ast::Expr::Literal(lit) => match lit {
                ...
                fe_sql_parser::ast::LiteralValue::Boolean(b) => {
        ...
        if stmt.is_global {
            ...
            self.with_session_mut(conn_id, |s| s.session_vars.set(&var_name, &value))?;
        }
    ...
    pub(crate) fn analyze_table(
        &self,
        _conn_id: u32,
        stmt: &fe_sql_parser::ast::AnalyzeTableStmt,
    ) -> Result<QueryResult, String> {
        Ok(QueryResult::with_rows(
            vec![ColumnDef {
                name: "Status".to_string(),
    ...
    pub(crate) fn drop_stats(
        &self,
        _conn_id: u32,
        stmt: &fe_sql_parser::ast::DropStatsStmt,
    ) -> Result<QueryResult, String> {
        Ok(QueryResult::with_rows(
            vec![ColumnDef {
                name: "Status".to_string(),
    ...
    pub(crate) fn create_function(
        &self,
        _conn_id: u32,
        stmt: &fe_sql_parser::ast::CreateFunctionStmt,
    ) -> Result<QueryResult, String> {
        Ok(QueryResult::with_rows(
            vec![ColumnDef {
                name: "Status".to_string(),
    ...
    pub(crate) fn drop_function(
        &self,
        _conn_id: u32,
        stmt: &fe_sql_parser::ast::DropFunctionStmt,
    ) -> Result<QueryResult, String> {
        Ok(QueryResult::with_rows(
            vec![ColumnDef {
                name: "Status".to_string(),
    ...
    pub(crate) fn create_job(
        &self,
        _conn_id: u32,
        stmt: &fe_sql_parser::ast::CreateJobStmt,
    ) -> Result<QueryResult, String> {
        Ok(QueryResult::with_rows(
            vec![ColumnDef {
                name: "Status".to_string(),
    ...
    pub(crate) fn drop_job_stmt(&self, _conn_id: u32, name: String) -> Result<QueryResult, String> {
        Ok(QueryResult::with_rows(
            vec![ColumnDef {
                name: "Status".to_string(),
                col_type: ColumnType::String,
    ...
    pub(crate) fn install_plugin(
        &self,
        _conn_id: u32,
        stmt: &fe_sql_parser::ast::InstallPluginStmt,
    ) -> Result<QueryResult, String> {
        Ok(QueryResult::with_rows(
            vec![ColumnDef {
                name: "Status".to_string(),
    ...
    pub(crate) fn drop_catalog_recycle_bin(
        &self,
        _conn_id: u32,
    ...
    pub(crate) fn create_sql_block_rule(
        &self,
        _conn_id: u32,
        stmt: &fe_sql_parser::ast::CreateSqlBlockRuleStmt,
    ) -> Result<QueryResult, String> {
        Ok(QueryResult::with_rows(
            vec![ColumnDef {
                name: "Status".to_string(),
            ...
            vec![vec![Some(format!(
                "CREATE SQL_BLOCK_RULE {} OK",
                stmt.name
            ))]],
        ))
    }

    pub(crate) fn alter_sql_block_rule(
        &self,
        _conn_id: u32,
        name: String,
        _props: Vec<(String, String)>,
    ) -> Result<QueryResult, String> {
        Ok(QueryResult::with_rows(
            vec![ColumnDef {
                name: "Status".to_string(),
    ...
    pub(crate) fn drop_sql_block_rule(
        &self,
        _conn_id: u32,
        name: String,
    ) -> Result<QueryResult, String> {
    ...
    pub(crate) fn create_row_policy(
        &self,
        _conn_id: u32,
        stmt: &fe_sql_parser::ast::CreateRowPolicyStmt,
    ) -> Result<QueryResult, String> {
        Ok(QueryResult::with_rows(
            vec![ColumnDef {
                name: "Status".to_string(),
    ...
    pub(crate) fn kill_analyze_job(&self, _conn_id: u32, id: String) -> Result<QueryResult, String> {
        Ok(QueryResult::with_rows(
            vec![ColumnDef {
                name: "Status".to_string(),
                col_type: ColumnType::String,
    ...
    pub(crate) fn cancel_export(&self, _conn_id: u32, id: String) -> Result<QueryResult, String> {
        Ok(QueryResult::with_rows(
            vec![ColumnDef {
                name: "Status".to_string(),
                col_type: ColumnType::String,
    ...
    pub(crate) fn pause_job(&self, _conn_id: u32, name: String) -> Result<QueryResult, String> {
        Ok(QueryResult::with_rows(
            vec![ColumnDef {
                name: "Status".to_string(),
                col_type: ColumnType::String,
    ...
    pub(crate) fn cancel_task(&self, _conn_id: u32, id: String) -> Result<QueryResult, String> {
        Ok(QueryResult::with_rows(
            vec![ColumnDef {
                name: "Status".to_string(),
                col_type: ColumnType::String,
