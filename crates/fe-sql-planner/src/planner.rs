...
impl Planner {
    ...
    pub fn plan(&self, stmt: Statement) -> Result<PlanNode, DrorisError> {
        match stmt {
            ...
            // Advanced SQL: DDL for Index (not yet implemented)
            Statement::CreateIndex(_) => Err(DrorisError::Internal("CREATE INDEX not yet implemented".to_string())),
            Statement::DropIndex(_) => Err(DrorisError::Internal("DROP INDEX not yet implemented".to_string())),
