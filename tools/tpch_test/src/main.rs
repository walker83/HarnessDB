use std::time::Instant;
use tpch_bench::{TpchBenchmark, queries};

fn main() {
    println!("=== RorisDB TPC-H Performance Test ===\n");

    let iterations = 5;
    println!("Running {} iterations for stable timing...\n", iterations);

    let all_queries = queries::all_queries();

    // Warm up
    let bench = TpchBenchmark::new_tiny();
    for idx in 1..=all_queries.len() {
        bench.run_query(idx);
    }

    // Header
    println!("| Q# | Query Name          | Status  | Avg (μs) | Min (μs) | Max (μs) |");
    println!("|----|---------------------|---------|----------|----------|----------|");

    let mut total_ok = 0;
    let mut total_err = 0;
    let mut total_min = u64::MAX;
    let mut total_max = 0u64;
    let mut total_avg = 0u64;

    for (idx, (name, _sql)) in all_queries.iter().enumerate() {
        let mut times: Vec<u64> = Vec::with_capacity(iterations);

        for _ in 0..iterations {
            let bench = TpchBenchmark::new_tiny();
            let start = Instant::now();
            let result = bench.run_query(idx + 1);
            let elapsed = start.elapsed().as_micros() as u64;

            if result.error.is_some() {
                times.push(u64::MAX);
            } else {
                times.push(elapsed);
            }
        }

        let q_min = *times.iter().min().unwrap_or(&0);
        let q_max = *times.iter().max().unwrap_or(&0);
        let q_avg = times.iter().filter(|&&t| t != u64::MAX).sum::<u64>() / iterations as u64;

        let status = if q_min == u64::MAX { "FAIL" } else { "OK" };
        if q_min == u64::MAX {
            total_err += 1;
        } else {
            total_ok += 1;
            total_min = total_min.min(q_min);
            total_max = total_max.max(q_max);
            total_avg += q_avg;
        }

        let short_name = name.replace("q", "Q");
        println!("| {:2} | {:18} | {:7} | {:8} | {:8} | {:8} |",
                 idx + 1, short_name, status, q_avg, q_min, q_max);
    }

    println!("\n=== Summary ===");
    println!("Total queries: {}", all_queries.len());
    println!("Passed: {}", total_ok);
    println!("Failed: {}", total_err);
    println!("\nOverall planning time (avg of all queries): {} μs", total_avg / total_ok.max(1));
}
